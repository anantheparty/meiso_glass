...
|---|---|---|
| `Edge` | 穿戴设备端 | K230D 平台；具体板卡与 runtime 基线待记录 |
...
- K230D 是当前 Edge SoC；具体板卡、runtime、toolchain、camera、display、IMU 与 Wi-Fi 接口必须从实机证据建立，不能由芯片名称推断。
...
SoC 变更首先影响 Edge 的 decode、render、format、compositor、display 与资源边界；当前 Host-only、Object、时间、pose 数学和进程内 adapter 不因 K230D 而改写，但目标编译与资源占用仍需实证。
...
3. 记录外部提供的 K230D 板卡/runtime/toolchain、可恢复性和接口基线；不在本项目内构建或刷写系统镜像。
4. 用 Host fixture 在 K230D 上逐个验证 Edge platform adapter；先验证受 SoC 变化影响的 compositor/display 边界，再接入 camera 与 pose source。
5. 对每个计划支持的 Composition source 分别完成 Edge-only profile；单侧缺失不阻塞另一侧工作。
6. 保持 Object、Composition 与业务逻辑不变，只把 fixture/进程内 adapter 换成双板标准 Wi-Fi + 同链路 UDP/IP carrier adapter。
7. 依次接入真实 camera、IMU 和 display；每次只增加一个硬件依赖，并注入 Host 延迟、断线、重启、丢包、旧版本和不完整 bulk。
8. 先测量 UDP reference profile；只有预先声明的延迟、功耗、CPU、丢包或调度指标显示明确收益时，才用同一 workload 验证 raw Wi-Fi adapter。

K230D 目标的具体板卡与传感器清单尚未记录。synthetic 或 recorded pose 只能验证协议与数学路径；完成真实 IMU 的时钟映射和外参标定后，才能声明硬件姿态修正成立。
...
- Edge-only profile 能从确定的 Host fixture 输入驱动 K230D 的同一平台接口，并在 Host 缺席或 session loss 时继续本地安全行为。
...
- K230D 与 Orin trace 能关联同一源 frame、Host overlay 版本、Edge 背景 frame 和最终修正结果。
