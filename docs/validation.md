...
Edge 操作系统与 compute domain 是平台选择，不属于产品契约。当前开发板按实际 BSP 和驱动能力使用：
...
5. 保持 object 与业务逻辑不变，只把 fixture/进程内 adapter 换成双板 Wi-Fi + UDP/IP carrier adapter。
