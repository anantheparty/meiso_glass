...
- 每个状态必须有 owner。
- Host 和 Device 的状态分开定义。
- Transport/profile ack 只说明 bytes 到达，不说明业务成功。
...
| `up` | 允许所有 channel，受 capability / policy 限制 | heartbeat、profile ack、retry control、统计 RTT/loss | loss/latency high -> `limited`; timeout -> `down` |
...
## 4. Host Session State

Owner：Host Runtime。

| State | Allowed Behavior | Self Behavior | Exit |
|---|---|---|---|
| `idle` | 创建 session；读取本地 config；不发送 Device command | 等待 app start 或 operator start | app opens session -> `linked` after link ready |
| `linked` | 发送 feature、scene、HUD、sensor request；接收 telemetry/fault | 发送 heartbeat；重试 unacked control；后台刷新 capability/time sync | link limited -> `limited`; timeout -> `lost`; close -> `idle` |
...
| `lost` | 不发新 feature request；可尝试 session resume | 标记 outstanding request unknown；等待 Device recovery 或 app close | session resumed -> `linked`; app close -> `idle` |
...
| `linked` | 接受 Host runtime request，但必须过 policy | 执行 lease expiry、telemetry、frame loop、asset request、heartbeat | link limited -> `limited`; Host lost -> `standalone`; fault -> `safe_mode` |
...
Device 是最终硬件 owner。Host 消失不等于 Device 停止；Device 必须继续 System HUD、thermal、battery 和安全路径。

## 6. Feature Lease State

Owner：Device Control Plane。

Feature lease 是 Device 侧状态。`rejected` 不是状态，只是 request 的结果。`degraded` 也不是单独状态，而是 `running` 的 granted params。
...
- Lease 到期后 Device 必须关闭高功耗 adapter，不等待 Host。
- Host lost 后 transient lease 必须释放，除非 profile 明确允许 offline continuation。

## 7. Sensor Subscription State

Owner：Device Data Plane。

Sensor subscription 是 Host 请求、Device 执行。启动 adapter 是 transition action，不需要单独 `starting` 状态。

| State | Allowed Behavior | Self Behavior | Exit |
|---|---|---|---|
| `off` | 无 sample；可接收 subscribe | 无 | subscribe accepted -> `streaming` |
| `streaming` | 发送 sample 或 local result | 按 rate 采样；按 channel 丢弃/发送；检查 lease | policy pause/link limited -> `paused`; unsubscribe/expiry -> `stopping` |
...
- 每个 sample 必须带 `subscriptionId`、runtime sequence 或 object version、timestamp。
- 高频 sample 使用 `latest` 或 media path，不能堵住 control。
- 订阅必须绑定 feature lease 或明确标记为不需要 lease。

## 8. Asset Cache State

Owner：Device Presentation Plane。Host 拥有 asset source/catalog，Device 拥有 cache state。
...
- 缺失 asset 不阻塞整个 scene；Device 显示 placeholder 并异步请求。
